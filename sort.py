...
def unpack(archive_path, path_to_unpack):
    ...
    unpack_folder = os.path.join(
        path_to_unpack, os.path.splitext(os.path.basename(archive_path))[0]
    )
...
def process_folder(folder_path):
    ...
    unpack_archives = (
        input("Do you want to unpack archives? (yes/no): ").strip().lower()
    )
    ...
    for root, dirs, files in os.walk(folder_path):
        for file in files:
            ...
            if file_ext in image_exts:
                ...
                new_file_name = get_unique_name(
                    os.path.join(folder_path, "images"),
                    normalize(file_name),
                    "." + file_ext.lower(),
                )
                shutil.move(
                    new_path, os.path.join(folder_path, "images", new_file_name)
                )
                ...
                shutil.move(
                    new_path, os.path.join(folder_path, "documents", new_file_name)
                )
                ...
                archive_dest_path = os.path.join(
                    folder_path, "archives", os.path.basename(new_path)
                )
                ...
                new_file_name = get_unique_name(
                    os.path.join(folder_path, "other"),
                    normalize(file_name),
                    "." + file_ext.lower(),
                )
